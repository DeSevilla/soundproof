...
pub fn additive_first(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1>>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let i1 = 0.01 * incr;
    let i5 = 0.05 * incr;
    let i10 = 0.1 * incr;
    // let i12 = 0.125 * incr;
...
pub fn all_harmonics(adsrs: &[(f32, f32, f32, f32)]) -> Vec<(f32, f32, f32, f32, f32)> {
    adsrs
        .iter()
        .enumerate()
        .map(|(i, (a, d, s, r))| (i as f32, *a, *d, *s, *r))
        .collect()
}

fn additive_array<T: AudioNode<Inputs = U1, Outputs = U1> + 'static>(
    instrument: An<T>,
    incr: f32,
    adsrs: &[(f32, f32, f32, f32, f32)],
) -> An<impl AudioNode<Inputs = U2, Outputs = U1> + use<T>> {
    let mut net = Net::wrap(Box::new((sink() | sink()) >> constant(0.0)));
    for (h, a, d, s, r) in adsrs.iter() {
        net =
            net & ((mul(*h) >> instrument.clone()) * adsr_live(*a * incr, *d * incr, *s, *r * incr))
    }
    unit(Box::new(net))
}

pub fn additive_second(
...
pub fn additive_decaying(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<(f32, f32, f32, f32, f32)> = (1..=10)
        .map(|h| h as f32)
        .map(|h| (h, 0.05, 1.01 - 0.1 * h, 1.0 / h, 1.0 - 0.1 * h))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn additive_increasing(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=10)
        .map(|h| h as f32)
        .map(|h| (h, 0.05, 0.1 * h, h * 0.1, 0.1 * h))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn attack_ascend(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=10)
        .map(|h| h as f32)
        .map(|h| (h, 0.05 * h, 0.1, 0.1, 0.1))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn attack_descend(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=10)
        .map(|h| h as f32)
        .map(|h| (h, 0.5 - 0.05 * h, 0.1, 0.1, 0.1))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn square_additive_one(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=10)
        .map(|h| h as f32)
        .map(|h| (h, h * h * 0.01, 1.0 - h * 0.1, 0.5 - h * 0.05, 0.1))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn additive_switchup(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=20)
        .filter(|x| *x == 1 || x % 3 == 0 || x % 5 == 0)
        .map(|h| h as f32)
        .map(|h| (h, 0.5 - h * 0.05, 1.0 - h * 0.1, 0.5 - h * 0.05, 0.1))
        .collect();
    additive_array(instrument, incr, &adsrs)
}

pub fn additive_another(
    instrument: An<impl AudioNode<Inputs = U1, Outputs = U1> + 'static>,
    incr: f32,
) -> An<impl AudioNode<Inputs = U2, Outputs = U1>> {
    let adsrs: Vec<_> = (1..=5)
        .map(|h| 2.pow(h) as f32)
        .map(|h| (h, 1.0 - h * 0.07, 1.0 - h * 0.07, 0.0, 0.0))
        .collect();
    additive_array(instrument, incr, &adsrs)
}
...
pub fn karplus() -> An<impl AudioNode<Inputs = U1, Outputs = U1>> {
    ...
    split()
        >> ((instrument | constant(0.25) | (pass() * 1.2) | constant(0.6))
            >> (lowpass_hz(BASE_HZ, 0.5) | multipass::<U3>())
            >> fbdf(-4.0))
        >> shape_fn(tanh)
        >> lowpass_hz(BASE_HZ, 1.0)
}
...
pub fn fm_sines(
    ...
    (split() >> ((mul(mult) >> sine()) * (base + sine_hz(amp_freq) * gap) * pass() * ratio))
        & pass() >> sine()
}
...
pub fn reverb_highpass() -> An<impl AudioNode<Inputs = U1, Outputs = U1>> {
    split::<U3>()
        >> (pass() + (reverb_stereo(30.0, 2.0, 0.5) >> join::<U2>() >> highpass_hz(150.0, 2.0)))
}

pub fn reverb_lowpass() -> An<impl AudioNode<Inputs = U1, Outputs = U1>> {
    split::<U3>()
        >> (pass() + (reverb_stereo(30.0, 2.0, 0.5) >> join::<U2>() >> lowpass_hz(300.0, 0.6)))
}

pub fn reverb_distort() -> An<impl AudioNode<Inputs = U1, Outputs = U1>> {
    split::<U3>()
        >> (pass() + (reverb_stereo(30.0, 2.0, 0.5) >> join::<U2>()))
        >> mul(5.0)
        >> shape_fn(tanh)
}
...
pub fn fbf(t: f32, db: f32) -> An<impl AudioNode<Inputs = U3, Outputs = U1>> {
    feedback((delay(t) | pass() | pass()) >> !lowpass() >> (mul(db_amp(db)) | mul(0.5) | mul(0.5)))
        >> (pass() | sink() | sink())
}

pub fn fbft(t: f32, db: f32) -> An<impl AudioNode<Inputs = U3, Outputs = U1>> {
    feedback(
        (((pass() | constant(t)) >> tap(1.0 / 10000.0, 5.0)) | pass() | pass())
            >> (!lowpass() * db_amp(db)),
    ) >> (pass() | sink() | sink())
}
...
pub fn delay_filter(db: f32) -> An<impl AudioNode<Inputs = U4, Outputs = U4>> {
    ...
    !((tap(1.0 / 10000.0, 5.0) | multipass::<U2>()) >> highpass())
        >> (mul(db_amp(db)) | mul(0.5) | mul(0.5) | mul(0.5))
