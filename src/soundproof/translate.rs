...
fn itype_translate_full(ii: usize, ctx: Context, term: &ITerm, depth: usize, mel: MelodySelector) -> Result<(Type, SoundTree), String> {
    ...
    match term {
        ITerm::Ann(ct, cty) => {
            ...
            let tytree = ctype_translate_full(ii, ctx.clone(), cty, Value::Star, depth + 1, mel.clone())?;
        ...
        ITerm::Pi(src, trg) => {
            let srctree = ctype_translate_full(ii, ctx.clone(), src, Value::Star, depth + 1, mel.clone())?;
        ...
        ITerm::App(f, x) => {
            let (fty, ftree) = itype_translate_full(ii, ctx.clone(), f, depth + 1, mel.clone())?;
            match fty {
                Value::Pi(src, trg) => {
                    let xtree = ctype_translate_full(ii, ctx, x, *src, depth + 1, mel.clone())?;
        ...
        ITerm::NatElim(motive, base, ind, k) => {
            let mtree = ctype_translate_full(ii, ctx.clone(), motive, Value::Pi(Box::new(Value::Nat), Rc::new(|_| Value::Star)), depth + 1, mel.clone())?;
            let m_val = motive.clone().eval(vec![]);
            let m_val1 = m_val.clone();
            let bctree = ctype_translate_full(ii, ctx.clone(), base, vapp(m_val.clone(), Value::Zero), depth + 1, mel.clone())?;
        ...
        ITerm::FSucc(n, fp) => {
            let ntree = ctype_translate_full(ii, ctx.clone(), n, Value::Nat, depth + 1, mel.clone())?;
            let n_val = n.clone().eval(vec![]);
            match &n_val {
                Value::Succ(m) => {
    ...
    match term {
        ...
        CTerm::Lam(body) => match ty {
            Value::Pi(src, trg) => {
                ...
                let subtree = ctype_translate_full(i + 1, new_ctx, &body.clone().subst(0, ITerm::Free(Name::Local(i))), trg(vfree(Name::Local(i))), depth + 1, mel.clone())?;
...
pub fn iterm_translate_full(term: &ITerm, depth: usize, mel: MelodySelector) -> SoundTree {
    ...
    match term {
        ITerm::Ann(cterm, cterm1) => SoundTree::simul(&[
            base_mel, 
            SoundTree::seq(&[
                cterm_translate_full(cterm, depth + 1, mel.clone()),
        ...
        ITerm::App(iterm, cterm) => SoundTree::simul(&[
            base_mel, 
            SoundTree::seq(&[
                iterm_translate_full(iterm, depth + 1, mel.clone()), 
        ...
        ITerm::NatElim(motive, base, ind, k) => SoundTree::simul(&[base_mel, SoundTree::seq(&[
            cterm_translate_full(motive, depth + 1, mel.clone()),
            cterm_translate_full(base, depth + 1, mel.clone()),
            cterm_translate_full(ind, depth + 1, mel.clone()),
        ...
        ITerm::FinElim(motive, base, ind, n, f) => SoundTree::simul(&[base_mel, SoundTree::seq(&[
            ...
            cterm_translate_full(base, depth + 1, mel.clone()),
            cterm_translate_full(ind, depth + 1, mel.clone()),
            cterm_translate_full(n, depth + 1, mel.clone()),
