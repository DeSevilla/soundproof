...
pub fn type_translate(term: &ITerm, meta: impl Selector) -> Result<SoundTree, String> {
    term.infer_translate(&Context::new(full_env()), meta)
        .map(|r| r.1)
}
    ...
    pub fn infer_translate(
        ...
        match self {
            ...
            ITerm::NatElim(motive, base, ind, k) => {
                let mtree = motive.check_translate(
                    ctx,
                ...
                let bctree =
                    base.check_translate(ctx, &vapp(m_val.clone(), Value::Zero), meta.clone())?;
            ...
            ITerm::FinElim(motive, base, ind, n, f) => {
                ...
                let bctree = base.check_translate(
                    ctx,
                    &Value::Pi(
                        Box::new(Value::Nat),
                        Rc::new(move |k| {
            ...
            ITerm::EqElim(a, motive, base, x, y, eq) => {
                ...
                let m_val = motive.eval(ctx);
                let m_val1 = m_val.clone();
                let btree = base.check_translate(
                    ctx,
                    &vpi(&a_val, move |x| {
                        vapp(
            ...
            CTerm::Lam(body) => match ty {
                Value::Pi(src, trg) => {
                    ...
                    let subtree = body
                        ...
                        .check_translate(&new_ctx, &trg(vfree(name)), meta.clone())?;
