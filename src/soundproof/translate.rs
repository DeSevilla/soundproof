...
pub fn type_translate(term: &ITerm, meta: impl Selector) -> Result<SoundTree, String> {
    term.infer_translate(Context::new(full_env()), meta).map(|r| r.1)
}
...
pub fn itype_translate(ctx: Context, term: &ITerm, meta: impl Selector) -> Result<(Type, SoundTree), String> {
    ...
    match term {
        ITerm::Ann(ct, cty) => {
            ...
            let tytree = cty.check_translate(ctx.clone(), &Value::Star, meta.clone())?;
        ...
        ITerm::Pi(src, trg) => {
            let srctree = src.check_translate(ctx.clone(), &Value::Star, meta.clone())?;
        ...
        ITerm::Free(name) => {
            ...
            let tytree = quote0(&ty).check_translate(ctx.clone(), &Value::Star, meta)?;
        ...
        ITerm::Succ(k) => {
            let subtree = k.check_translate(ctx, &Value::Nat, meta)?;
        ...
        ITerm::NatElim(motive, base, ind, k) => {
            let mtree = motive.check_translate(ctx.clone(), &Value::Pi(Box::new(Value::Nat), Rc::new(|_| Value::Star)), meta.clone())?;
            let m_val = motive.eval(ctx.clone());
            let m_val1 = m_val.clone();
            let bctree = base.check_translate(ctx.clone(), &vapp(m_val.clone(), Value::Zero), meta.clone())?;
            let indtree = ind.check_translate(
                ctx.clone(),
                &Value::Pi(Box::new(Value::Nat),
                    Rc::new(move |l| {let m_val2 = m_val1.clone(); Value::Pi(Box::new(vapp(m_val1.clone(), l.clone())), 
                        Rc::new(move |_| vapp(m_val2.clone(), Value::Succ(Box::new(l.clone()))))) }
                    )
                ),
                meta.clone()
            )?;
            let ktree = k.check_translate(ctx.clone(), &Value::Nat, meta)?;
        ...
        ITerm::FZero(n) => {
            let subtree = n.check_translate(ctx.clone(), &Value::Nat, meta)?;
            let n_val = n.eval(ctx.clone());
            Ok((Value::Fin(Box::new(Value::Succ(Box::new(n_val)))), SoundTree::simul([node_melody, subtree])))
        },
        ITerm::FSucc(n, fp) => {
            let ntree = n.check_translate(ctx.clone(), &Value::Nat, meta.clone())?;
            let n_val = n.eval(ctx.clone());
            match &n_val {
                Value::Succ(m) => {
        ...
        ITerm::FinElim(motive, base, ind, n, f) => {
            let mtree = motive.check_translate(ctx.clone(), &Value::Pi(Box::new(Value::Nat), Rc::new(|k| 
            ...
            let bctree = base.check_translate(ctx.clone(), &Value::Pi(Box::new(Value::Nat), Rc::new(
            ...
            let indtree = ind.check_translate(ctx.clone(), &Value::Pi(Box::new(Value::Nat), Rc::new(
                move |k| { 
            ...
            let ftree = f.check_translate(ctx.clone(), &Value::Fin(Box::new(n_val.clone())), meta)?;
        ...
        ITerm::Eq(a, x, y) => {
            ...
            let xtree = x.check_translate(ctx.clone(), &a_val, meta.clone())?;
        ...
        ITerm::EqElim(a, motive, base, x, y, eq) => {
            ...
            let xtree = x.check_translate(ctx.clone(), &a_val, meta.clone())?;
            let x_val = x.eval(ctx.clone());
            let ytree = y.check_translate(ctx.clone(), &a_val, meta.clone())?;
...
pub fn ctype_translate(ctx: Context, term: &CTerm, ty: &Type, meta: impl Selector) -> Result<SoundTree, String> {
    match term {
        CTerm::Inf(it) => {
            let i = ctx.bindings;
            let (ity, tree) = it.infer_translate(ctx, meta)?;
        ...
        CTerm::Lam(body) => match ty {
            Value::Pi(src, trg) => {
                ...
                let subtree = body.clone().subst(0, &ITerm::Free(name.clone())).check_translate(
                    new_ctx, &trg(vfree(name)), meta.clone()
                )?;
