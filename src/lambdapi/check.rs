    ...
    pub fn infer_type(&self, ctx: &Context) -> Result<Type, String> {
        match self {
            ...
            ITerm::Pi(src, trg) => {
                ...
                src.check_type(ctx, Value::Star)?;
                let src_ty = src.eval(ctx);
            ...
            ITerm::NatElim(motive, base, ind, k) => {
                ...
                motive.check_type(ctx, Value::Pi(Box::new(Value::Nat), Rc::new(|_| Value::Star)))?;
                let m_val = motive.eval(ctx);
                let m_val1 = m_val.clone();
                // base case must be a proof that the motive is true of 0
                base.check_type(ctx, vapp(m_val.clone(), Value::Zero))?;
            ...
            ITerm::FinElim(motive, base, ind, n, f) => {
                ...
                motive.check_type(ctx, Value::Pi(Box::new(Value::Nat), Rc::new(|k| 
                ...
                n.check_type(ctx, Value::Nat)?;
                // println!("validated n");
                let motive_val = motive.eval(ctx);
                let n_val = n.eval(ctx);
                ...
                base.check_type(ctx, Value::Pi(Box::new(Value::Nat), Rc::new(
                ...
                ind.check_type(ctx, Value::Pi(Box::new(Value::Nat), Rc::new(
                    move |k| { 
            ...
            ITerm::Eq(a, x, y) => {
                a.check_type(ctx, Value::Star)?;
                let a_val = a.eval(ctx);
                x.check_type(ctx, a_val.clone())?;
                y.check_type(ctx, a_val)?;
                Ok(Value::Star)
            },
            ITerm::Refl(a, x) => {
                a.check_type(ctx, Value::Star)?;
            ...
            ITerm::EqElim(a, motive, base, x, y, eq) => {
                ...
                x.check_type(ctx, a_val.clone())?;
                let x_val = x.eval(ctx);
                y.check_type(ctx, a_val.clone())?;
                let y_val = y.eval(ctx);
                eq.check_type(ctx, Value::Eq(Box::new(a_val), Box::new(x_val.clone()), Box::new(y_val.clone())))?;
