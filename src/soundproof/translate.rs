    ...
    pub fn infer_translate(&self, ctx: &Context, meta: impl Selector) -> Result<(Type, SoundTree), String> {
        ...
        match self {
            ...
            ITerm::Free(name) => {
                ...
                let tytree = quote0(&ty).check_translate(ctx, &Value::Star, meta)?;
                let tree = SoundTree::simul([node_melody, tytree]);
                Ok((ty, tree))
            },
            ITerm::App(f, x) => {
                let (fty, ftree) = f.infer_translate(ctx, meta.clone())?;
                match fty {
                    Value::Pi(src, trg) => {
            ...
            ITerm::FinElim(motive, base, ind, n, f) => {
                let mtree = motive.check_translate(ctx, &Value::Pi(Box::new(Value::Nat), Rc::new(|k| 
                ...
                let indtree = ind.check_translate(ctx, &Value::Pi(Box::new(Value::Nat), Rc::new(
                    move |k| { 
        ...
        match self {
            CTerm::Inf(it) => {
                ...
                if quote0(&ity) != quote0(ty) {
                    Err(format!("Expected {}, inferred {}, for term {}, level {i}", quote(i, ty), quote(i, &ity), self))
                }
                else {
                    Ok(tree)
                }
