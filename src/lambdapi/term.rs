...
impl ITerm {
    pub fn subst_free(self, name: &Name, new: &ITerm) -> ITerm {
        match self {
            ...
            ITerm::App(f, x) => {
                ITerm::App(Box::new(f.subst_free(name, new)), x.subst_free(name, new))
            }
            ITerm::Nat => self,
            ITerm::Zero => self,
            ITerm::Succ(cterm) => ITerm::Succ(cterm.subst_free(name, new)),
            ITerm::NatElim(motive, base, ind, k) => ITerm::NatElim(
                motive.subst_free(name, new),
                base.subst_free(name, new),
                ind.subst_free(name, new),
                k.subst_free(name, new),
            ),
            ...
            ITerm::FinElim(motive, base, ind, n, f) => ITerm::FinElim(
                ...
                base.subst_free(name, new),
                ind.subst_free(name, new),
                n.subst_free(name, new),
                f.subst_free(name, new),
            ),
            ...
            ITerm::Eq(a, x, y) => ITerm::Eq(
                a.subst_free(name, new),
                x.subst_free(name, new),
                y.subst_free(name, new),
            ),
            ITerm::Refl(a, x) => ITerm::Refl(a.subst_free(name, new), x.subst_free(name, new)),
            ITerm::EqElim(a, m, mr, x, y, eq) => ITerm::EqElim(
                a.subst_free(name, new),
                m.subst_free(name, new),
                mr.subst_free(name, new),
                x.subst_free(name, new),
                y.subst_free(name, new),
                eq.subst_free(name, new),
            ),
        }
    }
...
pub fn quote(i: usize, val: &Value) -> CTerm {
    match val {
        ...
        Value::Pi(src, trg) => itoc(ITerm::Pi(
            quote(i, src),
            quote(i + 1, &trg(vfree(Name::Quote(i)))),
        )),
...
pub fn neutral_quote(i: usize, neutral: &Neutral) -> ITerm {
    match neutral {
        ...
        Neutral::NatElim(motive, base, ind, k) => ITerm::NatElim(
            quote(i, motive),
            quote(i, base),
            quote(i, ind),
            itoc(neutral_quote(i, k)),
        ),
        Neutral::FinElim(motive, base, ind, n, f) => ITerm::FinElim(
            quote(i, motive),
            quote(i, base),
            quote(i, ind),
            quote(i, n),
            itoc(neutral_quote(i, f)),
        ),
        Neutral::EqElim(ty, motive, base, x, y, eq) => ITerm::EqElim(
            quote(i, ty),
            quote(i, motive),
            quote(i, base),
            quote(i, x),
            quote(i, y),
            itoc(neutral_quote(i, eq)),
        ),
    }
}
...
fn ite(name: &str, term: ITerm) -> (&str, Type, Value) {
    (
        name,
        term.infer_type(&Context::new(vec![])).unwrap(),
        term.eval(&Context::new(vec![])),
    )
}
...
pub fn std_env() -> Vec<(Name, Type, Option<Value>)> {
    ...
    let predata = vec![
        ...
        (
            ...
            clam(clam(clam(clam(ITerm::NatElim(
                ITerm::Bound(3).into(),
                ITerm::Bound(2).into(),
                ITerm::Bound(1).into(),
                ITerm::Bound(0).into(),
        ...
        (
            "FZero",
            vpi(Nat, |n| Fin(Box::new(Succ(Box::new(n))))),
            vlam(|n| FZero(Box::new(n))),
        ),
        (
            "FSucc",
            vpi(Nat, |n| {
                let n = n.clone();
                vpi(Fin(Box::new(n.clone())), move |_| {
                    Fin(Box::new(Succ(Box::new(n.clone()))))
                })
            }),
            vlam(|n| {
                let n = n.clone();
                vlam(move |f| FSucc(Box::new(n.clone()), Box::new(f)))
            }),
        ),
        (
            "Fin",
            vpi(Nat, |_| Star),
            vlam(|n| Fin(Box::new(n.clone()))),
        ),
        (
            "finElim",
            ipi(
                // motive
                ipi(ITerm::Nat, ipi(ITerm::Fin(bnd(0).into()), ITerm::Star)),
                ipi(
                    // motive at f0
                    ipi(
                        ITerm::Nat,
                        iapp(
                            iapp(bnd(1), ITerm::Succ(bnd(0).into())),
                            ITerm::FZero(bnd(0).into()),
                        ),
                    ),
                    ipi(
                        // motive at fsucc
                        ipi(
                            ...
                            ipi(
                                ...
                                ipi(
                                    iapp(iapp(bnd(3), bnd(1)), bnd(0)),
                                    iapp(
                                        iapp(bnd(4), ITerm::Succ(bnd(2).into())),
                                        ITerm::FSucc(bnd(2).into(), bnd(1).into()),
                                    ),
                                ),
                            ),
                        ),
        ...
        (
            "Not",
            vpi(Value::Star, |_| Value::Star),
            vlam(|x| vpi(x, |_| Value::Fin(Box::new(Value::Zero)))),
        ),
        (
            "Eq",
            vpi(Value::Star, |a| {
                vpi(a.clone(), move |x| vpi(a.clone(), |y| Value::Star))
            }),
            vlam(|a| {
                vlam(move |x| {
                    let a = a.clone();
        ...
        (
                ...
                ipi(
                    ...
                    ipi(
                        ...
                        ipi(
                            bnd(1),
                            ipi(
                                ITerm::Eq(bnd(2).into(), bnd(1).into(), bnd(0).into()),
                                ITerm::Star,
                            ),
                        ),
                    ),
                    ipi(
                        ...
                        ipi(
                            ...
                            iapps(
                                bnd(1),
                                [bnd(0), bnd(0), ITerm::Refl(bnd(2).into(), bnd(0).into())],
                            ),
                        ),
                        ipi(
                            // b
            ...
            .eval(&Context::new(vec![])), // clam(todo!()).eval(Context::new(vec![]))
        ),
        // ("girard", Value::Fin(Box::new(Value::Zero)), Some(girard_reduced())),
    ];
    predata
        .into_iter()
        .map(|(n, t, v)| (Name::Global(n.to_owned()), t, Some(v)))
        .collect()
}
...
fn test_stdlib() {
    for (name, ty, val) in std_env() {
        ...
        quoted
            .check_type(&Context::new(vec![]), ty.clone())
            .unwrap();
...
fn test_prelude() {
    ...
    let (typ, val) = ctx
        .find_free(&Name::Global("PowerfulUniverse".to_owned()))
        .unwrap();
    match typ {
...
fn test_file(file: &str) -> Context {
    ...
    for statement in stmts {
        ...
        match statement {
            ...
            _ => todo!(), // crate::parse::Statement::Eval(iterm) => todo!(),
                          // crate::parse::Statement::PutStrLn(_) => todo!(),
                          // crate::parse::Statement::Out(_) => todo!(),
                          // crate::parse::Statement::Set(tag, _) => todo!(),
                          // crate::parse::Statement::Command(_) => todo!(),
