        ...
        match self {
            ...
            ITerm::App(f, x) => {
                ...
                match fty {
                    ...
                    _ => Err(format!(
                        "Tried to call function {f} with non-function type {:?}",
                        quote(ctx.bindings, &fty)
                    )),
                }
            }
            ...
            ITerm::NatElim(motive, base, ind, k) => {
                ...
                ind.check_type(
                    ...
                    Value::Pi(
                        Box::new(Value::Nat),
                        Rc::new(move |l| {
                            let m_val2 = m_val.clone();
                            Value::Pi(
                                Box::new(vapp(m_val.clone(), l.clone())),
                                Rc::new(move |_| {
            ...
            ITerm::Bound(n) => Err(format!(
                "Not sure how to assign a type to Bound({n}), how did we get this?"
            )),
    ...
    pub fn check_type(&self, ctx: &Context, ty: Type) -> Result<(), String> {
        match self {
            CTerm::Inf(it) => {
                ...
                if infer != expect {
                    Err(format!(
                        "Mismatch: Received from callsite {:?}, inferred from term {:?}, for term {self}={self:?}, level {ii}",
                        expect, infer
                    ))
                } else {
                    Ok(())
            ...
            CTerm::Lam(body) => match ty {
                Value::Pi(src, trg) => {
                    ...
                    body.clone()
                        ...
                        .check_type(&new_ctx, trg(vfree(name)))
                }
