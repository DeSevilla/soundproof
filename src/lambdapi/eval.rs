...
fn nat_elim_rec(motive: &CTerm, base: &CTerm, ind: &CTerm, val: Value, ctx: &Context) -> Value {
    match val {
        ...
        Value::Succ(n) => vapp(
            vapp(ind.clone().eval(ctx), *n.clone()),
            nat_elim_rec(motive, base, ind, *n, ctx),
        ),
        Value::Neutral(neu) => Value::Neutral(Neutral::NatElim(
            Box::new(motive.eval(ctx)),
            Box::new(base.eval(ctx)),
            Box::new(ind.eval(ctx)),
            Box::new(neu),
        )),
...
fn fin_elim_rec(
    ...
    match val {
        ...
        Value::FSucc(k, g) => vapp(
            vapp(vapp(ind.clone(), *k), *g.clone()),
            fin_elim_rec(motive, base, ind, n, *g, ctx),
        ),
...
impl ITerm {
    pub fn eval(&self, ctx: &Context) -> Value {
        match self {
            ...
            ITerm::Free(x) => {
                if let Some((_, Some(v))) = ctx.find_free(x) {
                    v
                } else {
                    vfree(x.clone())
                }
            ...
            ITerm::NatElim(motive, base, ind, k) => {
                nat_elim_rec(motive, base, ind, k.eval(ctx), ctx)
            }
            ...
            ITerm::FinElim(motive, base, ind, n, f) => {
                fin_elim_rec(motive, base.eval(ctx), ind.eval(ctx), n, f.eval(ctx), ctx)
            }
            ITerm::Eq(a, x, y) => Value::Eq(
                Box::new(a.eval(ctx)),
                Box::new(x.eval(ctx)),
                Box::new(y.eval(ctx)),
            ),
            ...
            ITerm::EqElim(a, motive, base, x, y, eq) => {
                eq_elim_rec(a, motive, base.eval(ctx), x, y, eq.eval(ctx), ctx)
            }
        }
    }
}
