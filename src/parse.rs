...
pub fn reserved(name: &str) -> bool {
    match name {
        "let" | "forall" | "assume" | "putStrLn" | "out" => true,
        _ => false
    }
}
...
pub fn identifier(input: &str) -> IResult<&str, String> {
    ...
    if reserved(&ident) {
        // Err::F(nom::error::Error::new(input, ))
        // Err((input, nom::error::ErrorKind::Fail))
        // Err(nom::error::make_error(input, nom::error::ErrorKind::Fail))
...
pub fn lambda(vars: Vec<String>, i: &str) -> IResult<&str, CTerm> {
    ...
    //TODO whitespace it up elsewhere
...
pub fn cterm(n: usize, vars: Vec<String>, input: &str) -> IResult<&str, CTerm> {
    ...
    if n == 0 {
        ...
        // parens(|i| lambda(n, vars.clone(), i)).parse(input)
        context("cterm 1", alt((
            parens(|i| lambda(vars.clone(), i)),
            map(|i| iterm(n, vars.clone(), i), |iterm| CTerm::Inf(Box::new(iterm))),
            // map(|i| {
            //     let (i, iterm) = iterm(n, vars.clone(), i)?;
            //     Ok((i, CTerm::Inf(Box::new(iterm))))
            // }
        ))).parse(input)
    }
}
...
pub fn iterm(n: usize, vars: Vec<String>, input: &str) -> IResult<&str, ITerm> {
    ...
    if n == 0 {
        let rest = |i, t| {
            ...
            let (i, _) = tag("->").parse(i)?;
            let (i, _) = ws(i)?;
            let (i, body) = cterm(n, vars.clone(), i)?;
            let (i, _) = ws(i)?;
        ...
        panic!("Called iterm parser with too large a number")
    }
}
...
pub fn one_binding(use_env: bool, vars: Vec<String>, i: &str) -> IResult<&str, (String, CTerm)> {
    ...
    let (i, term) = cterm(0, if use_env { vars } else { vec![] }, i)?;
...
pub fn bindings(use_env: bool, vars: Vec<String>, input: &str) -> IResult<&str, (Vec<String>, Vec<CTerm>)> {
    ...
    let (i, bindings) = alt((
        many1(parens(|i| one_binding(use_env, vars.clone(), i))),
        (|i| one_binding(use_env, vars.clone(), i)).map(|b| vec![b])
    )).parse(input)?;
    //this part is fiddly
    // println!("got bindings: {bindings:?} || \x1b[32m{i}\x1b[0m");
    let (mut names, values): (Vec<String>, Vec<CTerm>) = bindings.into_iter().unzip();
    let mut vars = vars;
